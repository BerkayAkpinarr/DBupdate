...
def filterAll(enddate):
    ...
    for station in stations:
        ...
        print(station)
        curr.execute(
            f'SELECT snow_depth, m_date, stationid FROM aws_observation WHERE stationid = {station[0]} and m_date >= \'{enddate}\' ORDER BY m_date ASC')
        ...
        curr.executemany(
            'UPDATE aws_obs_filter SET snow_depth = %(snow_depth)s WHERE stationid = %(stationid)s and m_date = %(m_date)s',
            rows_to_be_updated)
        conn.commit()
