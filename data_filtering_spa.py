...
def filterSPA(enddate):
    conn = psycopg2.connect(
        "dbname = MGM user=mgm password = kalman host =192.168.0.221 port = 6666"
    )
    ...
    insert into spa_obs_filter(stationid, altitude, m_date, snow_depth, geom) select stationid, altitude, m_date, snow_depth, geom from spa_observation where m_date > '2021-11-25';
    ...
    for station in stations:
        ...
        curr.execute(
            f'SELECT snow_depth, m_date, stationid FROM spa_observation WHERE stationid = {station[0]} and m_date >= \'{enddate}\' and snow_depth is not NULL ORDER BY m_date ASC')
        ...
        curr.executemany(
            'UPDATE spa_obs_filter SET snow_depth = %(snow_depth)s , status=1 WHERE stationid = %(stationid)s and m_date = %(m_date)s',
            rows_to_be_updated)
        conn.commit()
